...
bool ExcelOp::AddSheet(int sheet_model_sn)
{
    ...
    else
    {
        ...
        sheet_rels_model_path = cache_path + WORKSHEETS_RELS_PATH + "sheet" + QString::number(sheet_model_sn) + ".xml.rels";
        if(QFile::exists(sheet_model_path)) sheet_rels_dst_path = cache_path + WORKSHEETS_RELS_PATH + "sheet" + QString::number(sheet_sn + 1) + ".xml.rels";
        else return false;
    }
    ...
    if(ret == false)
    {
        qDebug() << "in function AddSheet fail to load file: " << sheet_model_path;
        return false;
    }
    ...
    if(QFile::exists(sheet_rels_model_path))
    {
        ret = XmlOp::LoadXml(sheet_rels_model_path, sheet_rels_text);
        if(ret == false)
        {
            qDebug() << "fail to load file to sheet_rels_model_path: " << sheet_rels_model_path;
            return false;
        }
...
bool ExcelOp::DrawCell(int sheet_sn, QString image_path, QString from_col, QString from_row, QString to_col, QString to_row, double times)
{
    ...
    {
        QString tmp = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"></Relationships>";
    ...
    if(QFile::exists(sheetn_xml_rels_path) && sheetn_xml_rels_text.isEmpty()) sheetn_xml_rels_text = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"></Relationships>";
...
bool ExcelOp::WriteBatch(int sheet_sn, Info info, int direction)
{
    ...
    {
        QString tmp_label_sn = it->first;
        QString style = GetCellAttribute(sheet_sn, label_to_cell_sn_map[tmp_label_sn][0], "s");
        QString height = GetCellHeight(sheet_sn, label_to_cell_sn_map[tmp_label_sn][0]);

        if(style == "") style = "0";
        if(height == "") height = "12.8";
        ...
        QString tmp_label = XmlOp::GetSharedString(sharedstrings_text, tmp_label_sn);
        ReplaceSharedStringText(tmp_label, label_to_text_map[tmp_label_sn][0]); 
        for(int i = 1; i < it->second.size(); ++ i)
        {
            WriteCell(sheet_sn, label_to_cell_sn_map[tmp_label_sn][i], label_to_text_map[tmp_label_sn][i], style, height);
        }
    }
...
bool ExcelOp::DrawBatch(int sheet_sn, Info info, double times, int direction)
{
    ...
    for(auto it = label_to_image_path_map.begin(); it != label_to_image_path_map.end(); ++ it)
    {
        ...
        {
            std::vector<QString> col_and_row = CountRowAndCol(label_to_cell_sn_map[tmp_label_sn][i]);
            DrawCell(sheet_sn, label_to_image_path_map[tmp_label_sn][i], col_and_row[0], col_and_row[1], col_and_row[0], col_and_row[1], times);
        }
    }
    return true;
}
...
int ExcelOp::AddImage(int sheet_sn, QString image_path, int &image_height, int &image_width)
{
    ...
    if(QFile::exists(drawingn_xml_rels_path) && tmp_text.isEmpty()) tmp_text = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"></Relationships>";
